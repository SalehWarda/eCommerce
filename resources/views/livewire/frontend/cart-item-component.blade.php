    ...
    <tr x-data="{show:true}" x-show="show">
        <th class="ps-0 py-3 border-light" scope="row">
            <div class="d-flex align-items-center"><a class="reset-anchor d-block animsition-link"
                                                      href="{{route('frontend.product',$cartItem->model->slug)}}"><img
                        src="{{asset('images/products/'.$cartItem->model->firstMedia->file_name)}}"
                        alt="{{$cartItem->model->name}}" width="70"/></a>
                <div class="ms-3"><strong class="h6"><a class="reset-anchor animsition-link"
                                                        href="{{route('frontend.product',$cartItem->model->slug)}}">{{$cartItem->model->name}}</a></strong></div>
            </div>
        </th>
...
{{--                    <input class="form-control form-control-sm border-0 shadow-0 p-0" type="text" />--}}
        ...
        <td wire:click.prevent="removeFromCart('{{$cartItem->rowId}}')" x-on:click="show=false" class="p-3 align-middle border-light"><a class="reset-anchor" href="#!"><i
